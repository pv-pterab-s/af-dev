...
void rotate(Param<T> out, Param<T> in, float theta, af_interp_type method, int order) {
    ...
    getQueue().submit([&](sycl::handler &h) {
        ...
        h.parallel_for(sycl::nd_range{global, local}, [=](sycl::nd_item<2> it) {
            debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = 0;
        });
    }).wait();
    ...
    getQueue().submit([&](sycl::handler &h) {
        ...
        h.parallel_for(sycl::nd_range{global, local}, [=](sycl::nd_item<2> it) {
          ...
          int outoff     = setId * nimages * out.info.strides[2] + batch * out.info.strides[3];
          int inoff      = setId * nimages * in.info.strides[2] + batch * in.info.strides[3];
          const int loco = outoff + (yido * out.info.strides[1] + xido);

          // debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = limages;
          ...
          {
            int xid = (method == AF_INTERP_LOWER ? floor(xidi) : round(xidi));
            int yid = (method == AF_INTERP_LOWER ? floor(yidi) : round(yidi));
            ...
            // debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = loco; // + n * out.info.strides[batch_dim];
            // debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = ;

            for (int n = 0; n < limages; n++) {
              int idx_n = idx + n * in.info.strides[batch_dim];
              debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = idx_n;
              T val = (clamp || cond) ? d_in[idx_n] : zero;
              d_out[loco + n * out.info.strides[batch_dim]] = val;
              // debugBufferAcc[it.get_global_id(0)][it.get_global_id(1)] = -1;
              // loco + n *out.info.strides[batch_dim];
            }
          }
        });
    }).wait();
