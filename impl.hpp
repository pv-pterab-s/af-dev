...
template <typename T, typename aT>
void convNHelper(const conv_kparam_t<aT> &param, Param<T> &out, const Param<T> &signal,
...
#ifdef READ_NOT_WRITE
    printf("noope\n");
#else
    OPEN_W("test-data");
    WRITE(signal); WRITE(filter); WRITE(out); WRITE(kind); WRITE(rank); WRITE(expand);
#endif
